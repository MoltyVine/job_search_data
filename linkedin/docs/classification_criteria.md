...
- Classification: the Claude Code agent, working through the thread dump directly (via the `job-search-extraction` skill, or on request), or `--backend openrouter` for high-volume runs (needs `OPENROUTER_API_KEY`)
